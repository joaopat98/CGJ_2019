...
void Engine::setDisplayFn(void(*fn)(double elapsed_sec))
{
	display = fn;
}

void Engine::setKeyFn(void(*fn)(GLFWwindow* win, int key, int scancode, int action, int mods)) {
	glfwSetKeyCallback(win, fn);
}

void Engine::setCursorPosFn(void(*fn)(GLFWwindow* win, double xpos, double ypos)) {
	glfwSetCursorPosCallback(win, fn);
}
...
void Engine::setWindowSizeCallBack(void(*fn)(GLFWwindow*, int, int))
{
